...
public class Kagirl {
	...
	static final String jsKawaContent = "<script type='text/javascript'> " 
		      ...
		      +"     var note=getNote();                                         "
		      +"     var vtype=getQueryString('vtype');                          "
		      +"     var vid=getQueryString('vid'); var musicUrl='';var soundUrl='@3@'; if(null!=vtype&&''!=vtype&&null!=vid&&''!=vid) {var soundUrl='voice/'+vtype+'/'+vid+'.mp3';}                            "
		      +" </script>                                                       "
		      +"\n<script src=\"./js/heka.js";
	static final String kawaHost = "http://kwvk.oicp.net/1heka/kawaweika";

	public Kagirl() throws Exception {
	...
	public void processCard(String pageUrl) throws Exception {
		...
		pageSource = pageSource.replace("<script src=\"ymzg.js", jsReplaceContent);
	...
	public void processKawaCard(String pageUrl) throws Exception {
		...
		for(WebElement imgElement:imgElements) {
			String attributeImg = imgElement.getAttribute("src");
			if(null==attributeImg||attributeImg.contains("kawa1.gif")||attributeImg.contains("music_note_big.png")) {
				continue;
			}
		...
		pageSource = pageSource.replace(kawaHost, ".");
		pageSource = pageSource.replace("words = ", "words = note, //");
		pageSource = pageSource.replace("music              :", "music              : soundUrl, //");
		pageSource = pageSource.replace("gb2312", "utf-8");
...
//		ka.processCard("http://kwvk.oicp.net/1heka/kawaweika/y20019.php");
		//ka.processCatCard("http://kwvk.oicp.net/1heka/kawaweika/jieri.php");
		ka.processKawaCard("http://localhost/weitui/temp/20069.html?cardid=20069&modify=yes&code=01175b4418b09d5ff7e9c8ee2f52c5eu&state=0");
